...
from nil_adapter_template.compensation import COMPENSATIONS, compensate
from nil_adapter_template.manifest import apply_transport_quirks, overlay_requirements
from nil_adapter_template.state import ShimState
from nil_adapter_template.system import SystemClient, SystemError
from nil_adapter_template.translate import QUERY_VERBS, WRITE_VERBS, entity_ref
...
# (an invoice with line items) can only be built by its CURATED to_native. So the generic path must
# (a) accept flat fields and (b) delegate to the curated mapping when the target has one. This map +
# helper are the bridge that makes nil_intent work against a real, non-uniform backend.
...
def _native_for(target: str, data: dict[str, Any]) -> dict[str, Any]:
    """Shape a create payload for `target`: the curated to_native (handles invoice line-items etc.)
    when one is declared, else the raw fields. Falls back to raw if a curated mapping needs a field
    the caller did not supply (never a hard crash on the generic path)."""
...
def _prerequisite_gap(verb: Any, data: dict[str, Any], client: SystemClient) -> tuple[str, str] | None:
    ...
    mechanism (not a per-adapter shim). Two checks, both driven by what the verb declares:
      ...
      2. every referenced record (a foreign key like invoice.client_id) actually EXISTS.
...
def create_app(client: SystemClient, emitter: EventEmitter, *, bearer: str | None) -> FastAPI:
    app = FastAPI(title="nil-adapter-template shim", version="0.1.0")
    state = ShimState()
    ...
    def propose(env: dict[str, Any] = Body(...), authorization: str | None = Header(None)) -> dict[str, Any]:
        ...
        if isinstance(verb_name, str) and verb_name.startswith("resource."):
            ...
            if op == "create" and not data:
                return _refusal(env, "INVALID_ARGS", "missing record fields for create", field="data")
            ...
            return _envelope("PROPOSAL", env, {
                ...
                # editable at the decision card: the record fields (never target/id). The approver may
                # adjust them; approving re-PROPOSES the edited args (fresh preview) before commit.
                "modifiable": [f"data.{k}" for k in sorted(data)],
                "expires_at": _now()})
    ...
    def commit(env: dict[str, Any] = Body(...), authorization: str | None = Header(None)) -> dict[str, Any]:
        ...
        if stored.get("resource"):  # generic CRUD path — dispatch by args.target, synthesize result
            ...
            try:
                if op == "create":
                    # Delegate to the curated to_native so a structured entity (invoice line-items,
